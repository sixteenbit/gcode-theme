 ...
 * Force Login
 */
if ( ! class_exists( 'GT_Force_Login' ) ) {
	/**
	 * GT_Force_Login
	 */
	class GT_Force_Login {
		/**
		 * Initialization.
		 */
		...
		public static function forcelogin() {
			...
			if ( ! is_user_logged_in() ) {
				...
				if ( ! is_user_member_of_blog() && ! current_user_can( 'setup_network' ) ) {
					wp_die( __( "You're not authorized to access this site.", 'gcode-theme' ), get_option( 'blogname' ) . ' &rsaquo; ' . __( 'Error', 'gcode-theme' ) );
				}
			}
		}
		...
		public static function forcelogin_rest_access( $result ) {
			if ( null === $result && ! is_user_logged_in() ) {
				return new WP_Error( 'rest_unauthorized', __( 'Only authenticated users can access the REST API.', 'gcode-theme' ), array( 'status' => rest_authorization_required_code() ) );
			}
